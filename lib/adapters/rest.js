...
function RestAdapter(api, options) {
  this.api = api;
  this.options = options || (api.options || {}).rest;
  this._routes = [];
};
...
RestAdapter.prototype.createHandler =  function() {
  ...
  function createRoutes() {
    methods.forEach(function(method) {
      adapter._routes.push({
        verb: method.http.verb || 'all',
        path: method.fullPath(),
        handler: function(req, res, next) {
          var ctx = new HttpContext(req, res, method, adapter.options);
          adapter.invokeMethod(ctx, method, next);
        }
      });
    });
  };
  ...
  function applyRoutes() {
    adapter._routes.forEach(function(route) {
  ...
  debug('All Routers: %j', this.allRoutes());
  ...
  router.use(RestAdapter.errorHandler(this.api.options.errorHandler));
...
RestAdapter.prototype.invokeMethod = function(ctx, method, next) {
  ...
  async.series(
    [api.invokeMethodInContext.bind(this.api, ctx, method)],
    function(err) {
