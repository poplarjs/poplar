...
Poplar.set = Poplar.prototype.set = function(name, obj, mode) {
  ...
  switch(mode){
    case 'overwrite':
      ...
      break;
    case 'extend':
      var original = __locals[name];
      __locals[name] = util._extend(original, obj);
      break;
  }
};
...
Poplar.unset = Poplar.prototype.unset = function(name) {
  delete __locals[name];
};

/**
 * Returns the value of name app configuration, where name is one of strings in the app __locals table. For example:
...
Poplar.prototype.setBasePath = function(basePath) {
  assert(_.isString(basePath) && /^[a-zA-Z0-9_\/]+$/g.test(basePath), util.format('\'%s\' is not a valid basePath, basePath must be a string, \'a-z\', \'A-Z\' and _ are allowed' , basePath));
