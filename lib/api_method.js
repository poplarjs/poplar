...
ApiMethod.prototype.invoke = function(args, options, cb) {
  ...
  var ApiMethod = this.constructor;
  ...
  if (validationErrors && validationErrors.any()) {
    return callback(validationErrors.asJSON());
  }
    ...
    if (retval && typeof retval.then === 'function') {
      return retval.then(
        function(args) {
          ...
          debug('- %s - promise result %j', self.fullName(), result);
          cb(null, result);
        },
        cb // error handler
      );
