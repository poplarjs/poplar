...
ApiMethod.prototype.sanitize = function(args, cb) {
  var sanitizedArgs = invokeSantizers(args, this.accepts);
...
ApiMethod.prototype.validate = function(args, cb) {
  var validationErrors = invokeValidations(args, this.accepts);
  if (cb && typeof cb === 'function') {
...
ApiMethod.prototype.invoke = function(ctx, cb) {
  ...
  // expose helpers into method
  util._extend(self, ctx.helpers || {});
  ...
  if (accepts) {
    ...
    // Sanitize args
    formattedArgs = this.sanitize(formattedArgs, accepts);

    // Check validations
    validationErrors = this.validate(formattedArgs, accepts);
  }
...
/**
 * @class Method for invokation
 *
 * @constructor
 * Create a new `MethodInvocation` with the given `options`.
 */
function MethodInvocation(apiMethod) {
  this.__original = apiMethod.__original;
  this.fn = apiMethod.fn;
  this.name = apiMethod.name;
  this._apiBuilder = apiMethod._apiBuilder;
  this.accepts = apiMethod.accepts;
  this.returns = apiMethod.returns;
  this.presenter = apiMethod.presenter;
  this.presenterSource = apiMethod.presenterSource;
  this.description = apiMethod.description;
