...
function ApiMethod(name, options, fn) {
  ...
  if (this.presenterSource) {
    assert(util.isString(this.presenterSource), 'presenterSource must be a valid string');
  }
};
    ...
    if (path && util.isString(path)) {
      ...
      if (value) {
        list.forEach(function(el, i) {
          ...
          if (list.length === i + 1) {
            result[numeric(el)] = value;
          } else {
            result = result[numeric(el)];
          }
        });
        return source;
      } else {
